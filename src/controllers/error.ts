...
export const genericExpressErrorHandler = (err, req, res, next) => {
  ...
  console.error(
    `Custom Message: ${util.inspect(err.additionalInfo, true, 4, true)}`,
  );
  console.error(`System Message: ${err.systemMessage}`);
  if (Object.keys(err.params).length)
    console.error(`Params: ${util.inspect(err.params, true, 4, true)}`);
  if (Object.keys(err.query).length)
    console.error(`Query: ${util.inspect(err.query, true, 4, true)}`);
  if (Object.keys(err.body).length)
    console.error(`Body: ${util.inspect(err.body, true, 4, true)}`);
