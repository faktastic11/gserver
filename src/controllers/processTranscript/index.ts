...
export const initTranscriptProcessing = async (RawTranscriptId: string) => {
  ...
  const stagingTranscriptId =
    await processRawTranscriptToStaging(RawTranscriptId);
  // call pipeline function from staging --> process transcript
  const processedTranscriptIds =
    await processStagingTranscriptToProcessed(stagingTranscriptId);
