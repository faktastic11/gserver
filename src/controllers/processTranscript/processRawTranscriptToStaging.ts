...
export default async function processRawTranscriptToStaging(
  ...
  const { fiscalYear, fiscalQuarter, companyName, companyTicker } =
    rawTranscript;

  logger.info(
    `Start processing raw transcript ${rawTranscriptId} to staging transcript line items`,
  );
  ...
  if (useBinaryClassifier) {
    transcriptDocuments = await binaryClassification(
      companyName,
      transcriptDocuments,
    );
  }
  ...
  for (const doc of transcriptDocuments) {
    try {
      ...
      const [rawResult, response] = await extractChat.openAIGPTAPICall({
        prompt: extractPrompt,
      });
      if (rawResult != null) {
        for (const row of rawResult) {
          logger.debug(JSON.stringify(row));
          const lineItemEmbedding = await extractChat.getEmbeddings({
...
async function testProcessingTranscripts() {
  ...
  for (const ticker of tickers) {
    const rawTranscripts = await RawTranscript.findOne({
