...
const convertValues = (stagingLineItem: StagingLineItemType) => {
  ...
  if (rawScale.includes("million")) {
    ...
    logger.warn(
      `Unknown scale: ${rawScale}, low: ${rawLow}, high: ${rawHigh}, unit: ${rawUnit}}}`,
    );
  }

  return { scale, unit, amtLow, amtHigh };
};
export default async function processStagingTranscriptToProcessed(
  stagingTranscriptId: ObjectId,
): Promise<ObjectId[]> {
  logger.info(
    `Processing staging transcript ids: ${stagingTranscriptId} to processed transcript`,
  );

  const stagingTranscript =
    await StagingTranscript.findById(stagingTranscriptId);
  ...
  const { companyName, companyTicker, rawTranscriptId, stagingLineItems } =
    stagingTranscript;
