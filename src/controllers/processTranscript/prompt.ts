...
export class OpenAICompletion {
  ...
  constructor({ sessionId, prompts, chatCompletionResponse }) {
    ...
    const { message: resMessage, finish_reason } =
      this.chatCompletionResponse.choices[0];
    ...
    try {
      this.cost =
        this.promptTokens * perTokenCost[this.model].input +
        this.completionTokens * perTokenCost[this.model].output;
    } catch (err) {
      this.cost = 0;
    }
  }
