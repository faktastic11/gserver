...
const processTranscriptsValidation = (req, res, next) => {
  ...
  validateFn(req, res, next, [
    { schema: bodySchema, reqTarget: reqTargetTypes.BODY },
  ]);
};

router.post(
  "/v1/processTranscripts",
  processTranscriptsValidation,
  async (req, res, next) => {
    ...
    try {
      ...
      const processedTranscripts = await initTranscriptProcessing(
        rawTranscript.id,
      );
