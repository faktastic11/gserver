...
const parseLineItemValue = (row: CleanedRowType): { amtLow: number | null; amtHigh: number | null } => {
...
const parseTranscriptPosition = (transcriptPosition: string): { startLine: number; endLine: number } => {
...
const insertProcessedTranscripts = async (filePath, deleteCurrent = true) => {
  ...
  const rawTranscript = await RawTranscript.findOne({ companyTicker: ticker, fiscalQuarter, fiscalYear })
  console.log(rawTranscript?.id || 'no raw transcript found')

  // overwrite existing processed transcript data
  if (deleteCurrent) {
    await ProcessedTranscript.deleteMany({ rawTranscriptId: rawTranscript?.id })
  }
  ...
  for (const row of rows) {
    ...
    try {
      if (action?.toLowerCase().includes('delete') || !metricType?.toLowerCase().includes('guidance')) {
        continue
      }
      ...
      const newProcessedDoc = await ProcessedTranscript.create({
        ...
        value: {
          ...
          ...(amtLow && amtHigh && { mid: { amt: (amtLow + amtHigh) / 2, unit: rawUnit } }),
        },
  ...
  */
  ...
  const erroredTranscripts = await Promise.all(
    cleanedTranscriptNames.map(async (transcriptFileName) => {
      ...
      try {
        return await insertProcessedTranscripts(`${baseFilePath}/${transcriptFileName}`)
      } catch (err) {
        console.log(err)
