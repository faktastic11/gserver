...
const parseLineItemValue = (
  value: string,
): { amt: number | null; unit: string | null; qualitative: boolean } => {
  ...
  if (value[0] === "$") {
    return {
      amt: parseFloat(value.slice(1).replace(/,/g, "")),
      unit: "USD",
      qualitative: false,
    };
  }
  ...
  if (value[value.length - 1] === "%") {
    return {
      amt: parseFloat(value.slice(0, value.length - 1)),
      unit: "percent",
      qualitative: false,
    };
  }
...
const parseTimePeriod = (
  timePeriod: string,
): { fiscalQuarter: number | null; fiscalYear: number } => {
  ...
  inputStream
    ...
    .on("data", async (row: any) => {
      ...
      const {
        amt: amtLow,
        unit: unitLow,
        qualitative: qualitativeLow,
      } = parseLineItemValue(low);
      const {
        amt: amtHigh,
        unit: unitHigh,
        qualitative: qualitativeHigh,
      } = parseLineItemValue(high);
      const { fiscalQuarter: guidanceQtr, fiscalYear: guidanceYr } =
      ...
      const newProcessedDoc = await ProcessedTranscript.create({
        ...
        value: {
          ...
          ...(amtLow &&
            amtHigh && { mid: { amt: (amtLow + amtHigh) / 2, unit: unitLow } }),
